...
class InputFilter {
	...
	/** 
	  ...
	  * @param Array $attrArray - list of user-defined attributes
		* @param Array $attrMatchArray - list of regular expressions that have to match the attribute values
	  * @param int $tagsMethod - 0= allow just user-defined, 1= allow all but user-defined
	  * @param int $attrMethod - 0= allow just user-defined, 1= allow all but user-defined
	  * @param int $xssAuto - 0= only auto clean essentials, 1= allow clean blacklisted tags/attr
	  */
	public function __construct($tagsArray = array(), $attrArray = array(), $attrMatchArray = array(), $tagsMethod = 0, $attrMethod = 0, $xssAuto = 1) {		
		...
		$this->attrMatchArray = (array) $attrMatchArray;
	...
	protected function filterAttr($attrSet) {	
		...
		for ($i = 0; $i <count($attrSet); $i++) {
			...
			if (	((strpos(strtolower($attrSubSet[1]), 'expression') !== false) &&	(strtolower($attrSubSet[0]) == 'style')) ||
					...
					(strpos(strtolower($attrSubSet[1]), 'jscript:') !== false) ||
			...
			if ((!$attrFound && $this->attrMethod) || ($attrFound && !$this->attrMethod)) {
				// attr has value
				if (strlen($attrSubSet[1]) > 0) {
					if ($this->checkAttrValue($attrFound, $attrSubSet[0], $attrSubSet[1])) {
	...
	protected function checkAttrValue($found, $attr, $value) {
		if (!$found) return true; // Only check when we are using a whitelist
		$attrIndex = array_search(strtolower($attr), $this->attrArray);
		if (!isset($this->attrMatchArray[$attrIndex])) return true;
		return preg_match($this->attrMatchArray[$attrIndex], $value);
	}
