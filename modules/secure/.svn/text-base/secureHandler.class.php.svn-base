	...
	public function handler($match, array $pageSets) {
		...
		if (count($parts) > 0) {
			...
			if(strpos($fileName, '/', 1)) {
				$module = substr($fileName, 1, strpos($fileName, '/', 1));
			} else {
				$module = substr($fileName, 1);
			}
			if($module == '')
				$module = $lastPart;
		...
		foreach ($pageSets as $set) {
			foreach ($lastPart as $class => $file) {
				if (file_exists($_SERVER['DOCUMENT_ROOT'].'/../pages/'.$set.$fileName.$file)) {
					if(!Dispatcher::inst()->user->rights[$module]->access) {
						Dispatcher::header('/login?destination='.$_SERVER['REQUEST_URI']);
						die;
					}
